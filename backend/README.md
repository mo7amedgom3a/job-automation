...
├── cache/                   ← Redis-backed deduplication cache
├── storage/                 ← compatibility functions used by spider hooks
├── spiders/                 ← concrete Scrapling spiders
├── scheduler/               ← single-spider and cycle runner
├── config/                  ← site registry and runtime settings
└── usecases/                ← manual spider smoke-test scripts
```

Runtime services are defined in the root `docker-compose.yml`: `job-dork-api`, `postgres`, `redis`, and `n8n`.

`backend/` is the single source of truth for API routes, models, services, repositories, cache, spiders, and orchestration. Legacy packages such as `scraper_api/` must not be imported by backend code.

Search is Google API only. Backend search code uses `GoogleApiSearcher`, which calls Serper.dev first and SerpApi as fallback; non-Google search engines are intentionally not part of the backend.
...
graph TD
    ...
    L -->|8. Dedup Fingerprint| M[storage/db.py compatibility API]
    M -->|Delegates| N[repository/jobs.py]
    N -->|Unique Job| O[(PostgreSQL)]
```
...
```bash
cd backend
python3 -m usecases.spider_smoke_test --site linkedin_eg --max-pages 1
python3 -m usecases.spider_smoke_test --all --max-pages 1
```
...
Query PostgreSQL directly to verify the saved fields:

```bash
docker compose exec postgres psql -U job_user -d job_automation -c \
  "SELECT source, title, company, location FROM jobs WHERE source='myboard' LIMIT 5;"
```
